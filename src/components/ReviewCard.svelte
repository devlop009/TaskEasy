...
>
  ...
  <div class="h-[2px] bg-slate-400 w-1/4 mr-auto mb-4 mt-2"></div>
  <div class="flex items-center flex-wrap gap-2 text-xs sm:text-sm mb-4">
    {#each reviewItem.features as keywork}
      <div
        class="p-1 py-0.5 rounded-md border border-solid border-emerald-400 bg-emerald-50 text-slate-800"
      >
        <p>{keywork}</p>
      </div>
    {/each}
